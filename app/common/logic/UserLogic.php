...
class UserLogic
{
    ...
    public static function getList(array $params = [], bool $page = true, bool $model = false)
    {
        ...
        $list = UserModel::withSearch(['nickname', 'tel', 'status', 'pid', 'create_time'], $params)
            //->withoutField('')
            ->with(['PUser' => function ($query)
            {
                $query->field('id,avatar,nickname,tel');
            }])
            ->withCount([
                'UserInvite'
            ])
    ...
    public static function updateInfo(array $params, int $id)
    {
        try {
            $params['id'] = $id;
     ...
     * 用户余额更新
     * @param $params
     * @return Modelable|UserModel
     */
    public static function balanceUpdate($params): Modelable|UserModel
    {
    ...
    public static function qrcode(int $id)
    {
       $user = UserModel::find($id);
        ...
        $path = WechatMini::getWxAcodeunLimit('pages/index/index','from_id=' . $id, 'qrcode_' . $id . '.png');
    ...
    public static function getRechargeInfo(mixed $id)
    {
        $user = (new UserModel())->withCount(['Orders' => function (Query $query) {
            $query->where('order_type', OrderConstants::ORDER_TYPE_BUY)->where('payment_status', OrderConstants::PAYMENT_STATUS_PAID);
        }])->find($id);
