    ...
    apiService.enhanceResolution(imagePath, resolutionOption)
      .then(result => {
        ...
        if (result.success) {
          // 处理成功，跳转到预览页面
          wx.navigateTo({
            url: `/pages/preview/preview?imagePath=${result.data.processedImageUrl}&originalPath=${imagePath}&processType=resolution&scale=${resolutionOption}`
          });
        } else {
          // 后端API处理失败，尝试使用本地服务
    ...
    apiService.convertImage(imagePath, styleId)
      .then(result => {
        ...
        if (result.success) {
          // 处理成功，跳转到预览页面
          wx.navigateTo({
            url: `/pages/preview/preview?imagePath=${result.data.processedImageUrl}&originalPath=${imagePath}&processType=style&styleId=${styleId}&styleName=${styleName}`
          });
        } else {
          // 后端API处理失败，尝试使用本地服务
      ...
      imageProcessingService.enhanceImage(imagePath, 'resolution', { scale: option })
        .then(result => {
          ...
          if (result.success) {
            // 处理成功，跳转到预览页面
            wx.navigateTo({
              url: `/pages/preview/preview?imagePath=${result.tempFilePath}&originalPath=${imagePath}&processType=resolution&scale=${option}`
            });
          } else {
            // 处理失败
            wx.showToast({
      ...
      imageProcessingService.convertImageStyle(imagePath, styleType)
        .then(result => {
          ...
          if (result.success) {
            // 处理成功，跳转到预览页面
            wx.navigateTo({
              url: `/pages/preview/preview?imagePath=${result.tempFilePath}&originalPath=${imagePath}&processType=style&styleType=${styleType}`
            });
          } else {
            // 处理失败
            wx.showToast({
    ...
    apiService.createGif(imagePath, effectType)
      .then(result => {
        ...
        if (result.success) {
          // 处理成功，跳转到预览页面
          wx.navigateTo({
            url: `/pages/preview/preview?imagePath=${result.data.processedImageUrl}&originalPath=${imagePath}&processType=gif&effectType=${effectType}&styleName=${styleName}`
          });
        } else {
          // 后端API处理失败，尝试使用本地服务
          this.createLocalGif();
