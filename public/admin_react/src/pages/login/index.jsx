...
import loginMain from '@/static/login-index.png'
...
export default () => {
    ...
    const onFinish = (values) => {
        setLoading(true);
        adminUserApi.login(values).then((res) => {
            setLoading(false);
            if (res.code !== 1) {
    ...
    return <>
        ...
        <div className="ft">Copyright © {(new Date()).getFullYear()} {config.company} 版权所有 <a href="https://beian.miit.gov.cn/#/Integrated/index" target="_blank">{config.icp}</a></div>
    </>
}
